...
def picked_generator(item):
    ...
    while (i < 101):
        ...
        picked = lines[:i]
        fw = open('./output/picked/' + item + '/' + item + '_picked_' + str(i) + '.txt', 'w')
...
#set_list = ['pure_punctuation', 'pure_number_punctuation']
...
for item in set_list:
    ...
    for filename in filenames:
        ...
        corr = correlation_calc.correlation_cal('./analyze/' + analyzer_name +'analyze_' + item  + '.txt', './analyze/' + analyzer_name + 'analyze_' + filename)
        #print corr
        fr.write(str(corr) + '\n')
    fr.close()
